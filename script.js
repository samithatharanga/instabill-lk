...
document.addEventListener('DOMContentLoaded', () => {
    // ===================================================================================
    // InstaBill LK v11.1 (The Ultimate Retail & Data Sync Update) - FULLY CORRECTED
    ...
    const dom = {
        ...
        // CSV & Data Buttons (Dashboard)
        zReportBtn: document.getElementById('z-report-btn'), // CORRECTED ID
    ...
    function initializeApp() {
        console.log("InstaBill LK v11.1 Initializing...");
        registerAllEventListeners();
        loadAndApplySettings();
        renderAllLists();
        updatePreviews(); 
        checkOnlineStatus();
        renderSalesDashboard(); // FIX: Render dashboard on startup
    ...
    function registerAllEventListeners() {
        ...
        if(dom.itemsList) dom.itemsList.addEventListener('input', handleItemInput);
        if(dom.itemsList) dom.itemsList.addEventListener('click', handleItemClick);
        ...
        if(dom.printReceiptBtn) dom.printReceiptBtn.addEventListener('click', printReceipt);
        if(dom.holdBillBtn) dom.holdBillBtn.addEventListener('click', holdBill);
        if(dom.resumeBillsBtn) dom.resumeBillsBtn.addEventListener('click', () => openModal(dom.modals.heldBills, renderHeldBills));
        ...
        if(dom.scanBarcodeBtn) dom.scanBarcodeBtn.addEventListener('click', () => openModal(dom.modals.barcodeScanner, startBarcodeScanner));
        if(dom.dashboardExpenses) dom.dashboardExpenses.addEventListener('click', () => openModal(dom.modals.expenses, renderExpensesList));

        // Dashboard buttons
        if(dom.zReportBtn) dom.zReportBtn.addEventListener('click', generateAndShowZReport); // FIX: Correctly wired
        if(dom.downloadSalesCsvBtn) dom.downloadSalesCsvBtn.addEventListener('click', exportSalesToCsv);
        if(dom.closeRegisterBtn) dom.closeRegisterBtn.addEventListener('click', closeRegister);

        if(dom.downloadDebtorsCsvBtn) dom.downloadDebtorsCsvBtn.addEventListener('click', exportDebtorsToCsv);
        if(dom.exportInventoryBtn) dom.exportInventoryBtn.addEventListener('click', exportInventoryToCsv);
        if(dom.importInventoryBtn) dom.importInventoryBtn.addEventListener('click', () => dom.importInventoryInput.click());
        if(dom.importInventoryInput) dom.importInventoryInput.addEventListener('change', importInventoryFromCsv);

        if(dom.uploadLogoBtn) dom.uploadLogoBtn.addEventListener('click', () => dom.businessLogoUpload.click());
        if(dom.businessLogoUpload) dom.businessLogoUpload.addEventListener('change', handleLogoUpload);
        if(dom.receiptThemeSelect) dom.receiptThemeSelect.addEventListener('change', e => applyReceiptTheme(e.target.value));
        if(dom.addCashierForm) dom.addCashierForm.addEventListener('submit', addCashier);
        if(dom.backupDataBtn) dom.backupDataBtn.addEventListener('click', backupData);
        if(dom.restoreDataBtn) dom.restoreDataBtn.addEventListener('click', () => dom.restoreDataInput.click());
        if(dom.restoreDataInput) dom.restoreDataInput.addEventListener('change', restoreData);
        if(dom.clearDataBtn) dom.clearDataBtn.addEventListener('click', clearAllData);
        if(dom.addProductForm) dom.addProductForm.addEventListener('submit', addProductToCatalog);
        if(dom.addExpenseForm) dom.addExpenseForm.addEventListener('submit', addExpense);

        if(dom.productList) dom.productList.addEventListener('click', handleProductListActions);
        if(dom.creditLedgerList) dom.creditLedgerList.addEventListener('click', handleLedgerActions);
        if(dom.heldBillsList) dom.heldBillsList.addEventListener('click', handleHeldBillActions);
        if(dom.cashierList) dom.cashierList.addEventListener('click', handleCashierListActions);
    ...
    function renderReceipt(data) {
        ...
        const timeStr = data.dateTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    ...
    function finalizeAndSaveReceipt() {
        ...
        if (!data.customerName) { return showToast('Customer name is required for invoices/bills!', true); }
        ...
        renderSalesDashboard(); // FIX: Update dashboard after sale
        showToast('Transaction Finalized & Saved!', false);
        html2canvas(dom.receiptPreview, {scale: 2.5}).then(canvas => {
            const link = document.createElement('a');
            link.download = `${data.documentType}-${data.customerName.replace(/\s+/g, '_')}-${Date.now()}.png`;
            link.href = canvas.toDataURL();
            link.click();
        });
        resetForm();
    ...
    function importInventoryFromCsv(event) {
        ...
        reader.onload = e => {
            ...
            rows.forEach(row => {
                ...
                const product = { name: name, price: parseFloat(price), stock: parseInt(stock) || 0, barcode: barcode || '' };
    ...
    // --- DASHBOARD & REPORTING ---
    function generateAndShowZReport() {
        const today = new Date().toISOString().slice(0, 10);
        const todaysSales = state.salesHistory.filter(s => s.dateTime.startsWith(today) && s.documentType !== 'quotation');
        ...
        const paidSales = todaysSales.filter(s => s.status === 'paid').reduce((acc, sale) => acc + sale.total, 0);
        const creditSales = todaysSales.filter(s => s.status === 'credit').reduce((acc, sale) => acc + sale.total, 0);

        const reportContent = `
            <h3>Z-Report for: ${today}</h3>
            <p><strong>Total Sales Revenue:</strong> Rs. ${totalSales.toFixed(2)}</p>
            <p> - Paid In: Rs. ${paidSales.toFixed(2)}</p>
            <p> - On Credit: Rs. ${creditSales.toFixed(2)}</p>
            <p><strong>Total Expenses:</strong> Rs. ${totalExpenses.toFixed(2)}</p>
            <hr>
            <p><strong>Net Cash In Drawer:</strong> Rs. ${(paidSales - totalExpenses).toFixed(2)}</p>
        `;
        
        const reportModalContent = document.getElementById('z-report-content');
        if (reportModalContent) {
            reportModalContent.innerHTML = reportContent;
            openModal(dom.modals.zReport);
        } else {
            console.error("Z-Report content area not found!");
            showToast('Z-Report content area not found!', true);
        }
    }

    // THIS IS THE FULLY CORRECTED, COMPLETE FUNCTION
    function renderSalesDashboard() {
        ...
        // FIX: Use optional chaining to prevent errors if elements don't exist
